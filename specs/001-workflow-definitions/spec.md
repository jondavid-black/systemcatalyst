...
- Q: Use Case Reference Implementation? → A: **Loose Reference**. `use_case_id` is stored as a string field without a strict SQL Foreign Key constraint, avoiding dependencies on non-existent Use Case modules.
...
1. **Given** a workflow definition model, **When** I specify a Use Case ID (string), **Then** the model stores this association.
...
- **Missing References**: Since the Use Case ID is a loose reference, the validation logic simply checks if the field is populated (non-empty).
...
- **FR-002**: Workflow Definitions MUST include a traceable reference to a Use Case Definition (stored as a string ID).
...
- **WorkflowDefinition**: The container for a workflow, including ID, name, description, and Use Case reference (String).
